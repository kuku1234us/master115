...
-   **Worker Thread Logic (Option B):**
    ...
    4.  **Face Swap (Per Source Image):**
        ...
        -   **Result Handling:**
            ...
            -   **Revised Saving:** The worker thread should ideally get the image *bytes* directly if the service allows, or download the file to a known temporary location. Then, use `Pillow` within the thread to:
                ...
                -   Save as JPEG with appropriate quality (e.g., 75).
...
- [ ] **Task Manager:** Implement a thread-safe central manager class (e.g., using `threading.Lock` or `queue.Queue`) to track the status of all `(source_image, person_name, face_image)` swap tasks.
- [ ] **Dashboard Start Logic (Full):**
    - [ ] Modify the "Start" handler to get *all* selected `PersonBadge` data (person -> list of face paths).
    - [ ] Get *all* source image paths from `<AI Root Directory>/SourceImages/` (excluding `Completed/`).
    - [ ] Initialize the central task manager with all pending tasks.
    - [ ] Create a shared stop event (`threading.Event`).
    - [ ] For each selected face file across all selected persons:
        - [ ] Create a `FaceSwapWorker` instance, passing its assigned `person_name`, `face_image_path`, the *full list* of source image paths, and the shared stop event.
        - [ ] Connect worker signals to the dashboard/manager as needed.
        - [ ] Start the worker thread.
    - [ ] Update UI state (buttons, icons).
- [ ] **Worker Run Logic (Actual):**
    - [ ] Replace simulation logic in `FaceSwapWorker.run` with actual WebDriver automation:
        - [ ] Initialize WebDriver instance using the configured setup.
        - [ ] Navigate to the face swap service (Pixnova URL).
        - [ ] Perform the *one-time* upload of the worker's assigned `face_image_path`. Handle WebDriver waits and potential errors.
        - [ ] Loop through the `source_image_paths` list.
        - [ ] **Check Stop Event:** Before processing each source image, check the shared `stop_event.is_set()`. If set, break the loop.
        - [ ] Upload the *current* `source_image_path`.
        - [ ] Initiate the swap via WebDriver interactions.
        - [ ] Wait for the result and download it. Handle timeouts and service errors.
        - [ ] Use `Pillow` to open the downloaded image, convert to RGB if necessary, and save as JPG to `<AI Root Directory>/Temp/` with the correct naming convention. Handle image processing errors.
        - [ ] Emit `task_complete` or `task_failed` signal with relevant info (e.g., output path or error message).
        - [ ] **Notify Manager:** After successful save, notify the central task manager that this specific `(source_image, person_name, face_image)` task is done.
    - [ ] **Worker Cleanup:** Ensure `driver.quit()` is called in a `finally` block within the `run` method to close the browser even if errors occur or stop is signaled.
    - [ ] Emit `finished` signal when the `run` method exits.
- [ ] **Task Manager Logic:**
    - [ ] Implement the method to receive notifications from workers.
    - [ ] Track completed tasks for each source image.
    - [ ] When all faces for a given source image are reported as complete, move the original `source_image`